...
void test_arc(ei_surface_t surface, ei_rect_t* clipper)
{
	...
	uint32_t nb_points = 180;
	ei_point_t pts[nb_points];

	draw_arc(pts, nb_points, 125,125,100,0, 2*PI);
...
{
	ei_color_t color = {200,00,255,255};
	ei_point_t* pts = rounded_frame(&(ei_rect_t){{400,400},{100,100}}, 10, HAUT);
	ei_draw_polyline(surface,pts,23, color,clipper);
}
/*
 * ei_main --
 *
