...
ei_point_t* rounded_frame(ei_rect_t* cadre, uint32_t rayon, partie_button partie)
{
    // allocation conditionnelle de la memoire
    ei_point_t* points = (partie == COMPLET) ? NULL : (ei_point_t*)calloc((2 * rayon + 3), sizeof(ei_point_t));
    ...
    {
        case HAUT:
            draw_arc(points,rayon, top_left.x + largeur - rayon, top_left.y  + rayon, rayon/2, DEG_TO_RAD(45), DEG_TO_RAD(90));
            draw_arc(points + (rayon/2)*sizeof(ei_point_t),rayon, top_left.x + rayon, top_left.y + rayon, rayon, DEG_TO_RAD(90), DEG_TO_RAD(180));
            draw_arc(points + (3*rayon/2)*sizeof(ei_point_t),rayon/2, top_left.x + rayon, top_left.y + hauteur - rayon, rayon, DEG_TO_RAD(180), DEG_TO_RAD(225));
            points[2*rayon] = pt1;
        ...
        case  BAS:
            break;
        default:
            rounded_frame(cadre, rayon, HAUT);
            rounded_frame(cadre, rayon, BAS);
    }
