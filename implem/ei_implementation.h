...
#define CODE_GAUCHE  8   // bit 3
  ...
  * @param pixel_ptr pointeur vers les pixels de l'ÃƒÂ©cran
  * @param dimension dimension de la fenÃƒÂªtre ,pour calculer la position du point
  * @param point point Ãƒ  colorier
  * @param color couleur choisie
  */
 void draw_point(uint32_t* pixel_ptr, ei_size_t dimension, ei_point_t point, ei_color_t* color);

/** @brief retoune un boolÃƒÂ©en selon si point est Ãƒ  l'intÃƒÂ©rieur du clipper ou pas
 * @param point point pour lequel on veut vÃƒÂ©rifier la position
 ...
 */
 ...
 /// @param cliquee bool qui intervertit les couleurs du bouton pour lui donner cet effet de cliquÃƒÂ© ou pas
...
/// @brief CrÃƒÂ©e un pointeur qui pointe vers un ensemble de points qui permettent de dessiner un arc
...
/// @param y0 ordonnÃƒÂ©e du centre de l'arc
/// @param rayon rayon de l'arc
/// @param angle_debut angle de dÃƒÂ©but "sens trigo"
...
/// @brief CrÃƒÂ©e un pointeur qui pointe vers un ensemble de points qui permettent de dessiner le frame du bouton "un rectangle avec les coins arrondis"
...
/// @param partie structure qui dÃƒÂ©cide si on veut que le bas, le haut ou les deux "plus beau que faire des requÃƒÂªtes conditionnelles avec des ou et des and"
...
/// @param foncee bool qui dÃƒÂ©cide si on veut une couleur foncee ou pas
...
// prototypes des fonctions utilisÃƒÂ©s pour le dessin d'un polygone

/// @brief  Construit la table de cÃƒÂ´tÃƒÂ©s
/// @param point_array pointeur vers un tableau des sommets de notre cher polygone
/// @param point_array_size le nombre de sommets
/// @param y_min c'est la plus petite ordonnÃƒÂ©e des sommets du polygone "on veut travailler qu'entre y_min et y_max"
/// @param y_max c'est la plus grande ordonnÃƒÂ©e des sommets du polygone
/// @return la table de cÃƒÂ´tÃƒÂ©s
TC_t* construire_TC(ei_point_t* point_array, uint32_t point_array_size, int y_min, int y_max);

/// @brief supprime le cote dÃƒÂ¨s qu'on atteint son y_max
/// @param TCA table de cÃƒÂ´tÃƒÂ©s actifs
...
/// @return retourne la ligne de TC triÃƒÂ©e
TC_line_table* tri_line_table(TC_line_table* table);

/// @brief Colorie les pixels qui se trouvent dans nos scanlines entre 2 cÃƒÂ´tÃƒÂ©s adjacents
...
/// @brief met Ãƒ  jour les x__ymin dans TCA
/// @param TCA TCA qu'on veut mettre Ãƒ  jour
...
/// @brief Ajoute un segment Ãƒ  une ligne dans une TC
/// @param TC la TC en question
/// @param point1 extremitÃƒÂ© du segment
/// @param point2 extremitÃƒÂ© du segment
/// @param y_min ordonnÃƒÂ©e minimale du segment
...
/// @brief crÃƒÂ©e un segment qu'on veut ajouter dans la ligne de TC
/// @param point1 extremitÃƒÂ© du segment
/// @param point2 extremitÃƒÂ© du segment
/// @return retourne une variable de type segment qu'on veut ajouter Ãƒ  notre ligne de TC "voir struc segment"
...
/*******************************************************************************************************************************************/
  ...
  /**
   * @brief Colorie les pixels entre origine et extrÃƒÂ©mitÃƒÂ© en respectant l'algorithme de Bresenham
   *
   * @param origine Point de dÃƒÂ©part Ãƒ  colorier /origine de la ligne
   * @param extremite Point suivant Ãƒ  colorier / extrÃƒÂ©mitÃƒÂ© de la ligne
...
typedef struct ei_impl_frame_t {
  ...
  /*spÃƒÂ©cificitÃƒÂ©s communs au button et Ãƒ  la frame */
  ei_size_t* requested_size;//-----
  ei_color_t*	color;//----- la couleur de la frame
  int*	border_width;//----- l'ÃƒÂ©cart pour crÃƒÂ©er le relief
  ei_relief_t* relief;//----- le type de relief
  ei_string_t text;//----- le texte Ãƒ  ecrire
  ei_font_t* text_font;//----- le font qui est donnÃƒÂ© par le programmeur ou hw_text_font_create
...
/**
 * @brief Calcule lâ€™intersection de deux rectangles.
 *
 * @param rect1 Le premier rectangle.
 * @param rect2 Le second rectangle.
 * @return Un pointeur vers un rectangle reprÃ©sentant lâ€™intersection, ou NULL sâ€™il nâ€™y a pas dâ€™intersection.
 */
